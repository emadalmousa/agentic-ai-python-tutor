...
const LEVEL_LABEL_KEYS: Record<LevelKey, TranslationKey> = {
  beginner:     "levelTest.beginner",
  intermediate: "levelTest.intermediate",
  advanced:     "levelTest.advanced",
}
...
export default function LevelTestModal({ level, onClose, onTestResult }: Props) {
  ...
  const STEP_LABELS = [t("levelTest.stepMC"), t("levelTest.stepCodeReading"), t("levelTest.stepMiniTask")] as const
  ...
  useEffect(() => {
    generateLevelTest(level, token)
      ...
      .catch(() => setError(t("levelTest.generateError")))
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setError(t("levelTest.submitError"))
    } finally {
      setSubmitting(false)
    }
  }
  ...
  function handleRetry() {
    ...
    generateLevelTest(level, token)
      .then((data) => setTestData(data))
      .catch(() => setError(t("levelTest.retryError")))
      .finally(() => setLoading(false))
  }
        ...
        <div className={`px-6 py-4 border-b flex items-center justify-between ${dark ? "border-[#1e2f45] bg-[#0d1929]" : "border-gray-100 bg-white"}`}>
          <div>
            <h2 className={`text-base font-bold ${dark ? "text-white" : "text-gray-900"}`}>
              {t("levelTest.title", { label: t(LEVEL_LABEL_KEYS[level]) })}
            </h2>
            {!result && !loading && (
              <p className={`text-xs mt-0.5 ${dark ? "text-gray-500" : "text-gray-400"}`}>
                {t("levelTest.step", { step })} — {STEP_LABELS[step - 1]}
              </p>
            )}
          </div>
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {loading && (
            <div className="flex flex-col items-center py-12 gap-3">
              ...
              <p className={`text-sm ${dark ? "text-gray-400" : "text-gray-500"}`}>{t("levelTest.generating")}</p>
            </div>
          )}
            ...
            <div className="space-y-5">
              <div className="text-center py-4">
                ...
                <p className={`text-sm mt-1 ${dark ? "text-gray-400" : "text-gray-500"}`}>
                  {result.passed ? t("levelTest.passedMsg") : t("levelTest.failedMsg")}
                </p>
              </div>

              <div className={`rounded-xl border p-4 ${dark ? "border-[#1e2f45] bg-[#0a1525]" : "border-gray-200 bg-gray-50"}`}>
                <p className={`text-xs font-medium uppercase tracking-wider mb-3 ${dark ? "text-gray-500" : "text-gray-400"}`}>{t("levelTest.results")}</p>
                <div className="grid grid-cols-3 gap-3">
                  <ScoreBox label="MC" score={result.mc_score} max={30} dark={dark} />
                  <ScoreBox label={t("levelTest.stepCodeReading")} score={result.code_reading_score} max={30} dark={dark} />
                  <ScoreBox label={t("levelTest.stepMiniTask")} score={result.mini_task_score} max={40} dark={dark} />
                </div>
              </div>
              ...
              {!result.passed && result.per_question_feedback.length > 0 && (
                <div className="space-y-2">
                  {result.per_question_feedback.map((fb, i) => (
                    <div key={i} className={`rounded-lg p-3 text-sm border-s-4 ${fb.correct
          ...
          {!loading && !result && testData && (
            <>
              ...
              {step === 2 && (
                <div className="space-y-4">
                  <p className={`text-sm font-semibold ${dark ? "text-gray-200" : "text-gray-700"}`}>{t("levelTest.codeReadingInstruction")}</p>
                  ...
                  <textarea value={codeReadingAnswer} onChange={(e) => setCodeReadingAnswer(e.target.value)}
                    className={inputCls} rows={3} placeholder={t("levelTest.yourAnswer")} />
                </div>
              )}
              ...
              {step === 3 && (
                <div className="space-y-4">
                  <p className={`text-sm font-semibold ${dark ? "text-gray-200" : "text-gray-700"}`}>{t("levelTest.miniTaskTitle")}</p>
                  ...
                  {testData.test_data.mini_task.expected_output && (
                    <div className={`rounded-xl p-3 ${dark ? "bg-[#0a1525] border border-[#1e2f45]" : "bg-gray-50 border border-gray-200"}`}>
                      <p className={`text-xs uppercase tracking-wider mb-1 ${dark ? "text-gray-500" : "text-gray-400"}`}>{t("levelTest.expectedOutput")}</p>
              ...
              <div className="flex items-center justify-between pt-2">
                ...
                {step < 3 ? (
                  ...
                  <button onClick={handleSubmit} disabled={submitting}
                    ...
                    {submitting ? t("levelTest.submitting") : t("levelTest.submitBtn")}
                  </button>
                )}
              </div>
            </>
