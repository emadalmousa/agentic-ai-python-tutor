...
def analyze_code(request: CodeRequest) -> TutorResponse:
    result = run_analysis(request.code, request.question)
...
_CLASSIFY_SYSTEM = SystemMessage(content=(
    "Du klassifizierst Nachrichten. "
    "Antworte NUR mit 'ja' wenn die Nachricht mit Python-Programmierung zu tun hat, "
    "sonst NUR mit 'nein'. Kein weiterer Text."
))
...
def _is_python_related(message: str) -> bool:
    llm = get_classifier_llm()
    response = llm.invoke([_CLASSIFY_SYSTEM, HumanMessage(content=message)])
    return response.content.strip().lower().startswith("ja")


@router.post("/chat", response_model=ChatResponse)
def chat(request: ChatRequest) -> ChatResponse:
    if not _is_python_related(request.message):
