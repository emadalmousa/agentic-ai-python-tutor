"""
Tests: LangChain explain_code_tool and debug_code_tool.

Validates:
- explain_code_tool returns a string and delegates LLM instantiation to get_llm()
...
class TestExplainCodeTool:
    ...
    def test_explain_tool_invokes_llm_once(self):
        """explain_code_tool.invoke() calls llm.invoke exactly once."""
        import agent.tools.explain_tool as explain_module
    ...
    def test_explain_tool_includes_code_in_human_message(self):
        """The code must appear in the HumanMessage sent to the LLM."""
        import agent.tools.explain_tool as explain_module
        from langchain_core.messages import HumanMessage

        mock_llm = _make_mock_llm("Erklärung …")

        with patch.object(explain_module, "get_llm", return_value=mock_llm):
            explain_module.explain_code_tool.invoke({"code": "x = 42"})

        call_args = mock_llm.invoke.call_args
        messages = call_args[0][0]
        human_messages = [m for m in messages if isinstance(m, HumanMessage)]
        assert human_messages, "No HumanMessage found in llm.invoke call"
        assert "x = 42" in human_messages[0].content
...
class TestParseDebugResponse:
    ...
    def test_parse_fenced_json_with_braces_in_suggestion(self):
        ...
        assert result["error_found"] is True
        assert result["suggestion"] == "Fehler {liegt hier}"
    ...
    def test_parse_fallback_on_garbage(self):
        ...
        assert result == {"error_found": False, "error_type": "Kein Fehler", "suggestion": "Analyse nicht möglich."}
